...
#Table visualization of frequency of missing values
#fish_data |>
  #mutate(weight = if_else(is.na(weight),
                           #"missing",
                           #"present")) |>
  #group_by(year, section, trip, weight) |>
  #summarize(count = n(),
            #.groups = "drop") |>
  #pivot_wider(id_cols = year:trip,
              #names_from = weight,
              #values_from = count,
              #values_fill = list(count = 0), #used pivot_wider help description
             # names_prefix = "freq_weight_data_")
...
*In the 5th line, the code should read min(weight, na.rm = TRUE) instead of min(length, na.rm = TRUE)*
...
rescale_01 <- function(x){
  ...
  if(length(x) <= 1) {
    stop("Input must be greater than 1")}
  (x - min(x, na.rm = TRUE)) /
...
rescale_column <- function(df, x, y) {
  df |>
    mutate(rescale_length = rescale_01({{x}}),
           rescale_weight = rescale_01({{y}}))
...
rescale_column(fish_data, length, weight)
